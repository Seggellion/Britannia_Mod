...
public class WraithEntity extends Monster implements GeoAnimatable {
    ...
    public void registerControllers(AnimatableManager.ControllerRegistrar controllers) {
        controllers.add(new AnimationController<WraithEntity>(this, "controller", 0, this::predicate));
    }

private PlayState predicate(AnimationState<WraithEntity> state) {
    if (state.isMoving()) {
    ...
    // Spawn conditions
    public static boolean canSpawn(EntityType<WraithEntity> type, ServerLevelAccessor world, MobSpawnType spawnReason, BlockPos pos, RandomSource random) {
        //return Monster.isDarkEnoughToSpawn(world, pos, random) && Monster.checkMobSpawnRules(type, world, spawnReason, pos, random);
         //   LOGGER.info("Wraith canSpawn called at position {}: always returning true for testing", pos);
    return true;
    }


// Sound events for Wraith
    @Override
    ...
    protected void playStepSound(BlockPos pos, BlockState blockIn) {
        // Wraiths don't have typical footsteps, can use a wing flap sound here if needed.
...
protected ResourceKey<LootTable> getDefaultLootTable() {
    return ResourceKey.create(
        ...
        ResourceLocation.fromNamespaceAndPath("britannia_mod", "entities/wraith")
    );
}
