...
/**
 * HTTP client for {@code POST /api/banking/withdrawal/prepare}, structurally identical to {@link
 * BankingDepositClient} for that one action (the same three injectable seams, the same
 * production collaborators). {@code confirm}/{@code cancel} are not reimplemented here --
 * those two actions are entirely operation-type-agnostic on the wire (same request shape,
 * same result types, same endpoints as deposit's), so this class composes a {@link
 * BankingDepositClientPort} and delegates straight through rather than duplicating
 * already-tested dispatch logic for a second time.
 */
public final class BankingWithdrawalClient implements BankingWithdrawalClientPort {
    ...
    public BankingWithdrawalClient() {
    ...
    @Override
    public CompletableFuture<BankingWithdrawalPrepareResult> prepareWithdrawal(MinecraftServer server, BankingWithdrawalPrepareRequest request) {
        Objects.requireNonNull(request, "request");
        return dispatch(
                server, RailsApiUrlResolver.Endpoint.BANKING_WITHDRAWAL_PREPARE, serializePrepare(request),
                BankingTransferResponseParser::parseWithdrawalPrepare,
                BankingWithdrawalPrepareResult.LocalFailure::new, BankingWithdrawalPrepareResult.TransportFailure::new
        );
    }
    ...
    private static byte[] serializePrepare(BankingWithdrawalPrepareRequest request) {
