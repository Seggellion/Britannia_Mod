...
/**
 * HTTP client for {@code POST /api/banking/currency/withdrawal/prepare}, structurally identical
 * to {@link BankingCurrencyDepositClient}: the same three injectable seams and production
 * collaborators for its one prepare action, and the same composition for {@code
 * confirm}/{@code cancel} -- entirely operation-type-agnostic on the wire, so this class
 * delegates them to a composed {@link BankingDepositClientPort} rather than duplicating
 * already-tested dispatch logic a third time.
 */
public final class BankingCurrencyWithdrawalClient implements BankingCurrencyWithdrawalClientPort {
    ...
    public BankingCurrencyWithdrawalClient() {
    ...
    public CompletableFuture<BankingCurrencyWithdrawalPrepareResult> prepareCurrencyWithdrawal(
        ...
        return dispatch(
                server, RailsApiUrlResolver.Endpoint.BANKING_CURRENCY_WITHDRAWAL_PREPARE, serializePrepare(request),
                BankingTransferResponseParser::parseCurrencyWithdrawalPrepare,
                BankingCurrencyWithdrawalPrepareResult.LocalFailure::new, BankingCurrencyWithdrawalPrepareResult.TransportFailure::new
        );
    }
    ...
    private static byte[] serializePrepare(BankingCurrencyWithdrawalPrepareRequest request) {
