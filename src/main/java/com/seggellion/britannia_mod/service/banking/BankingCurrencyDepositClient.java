...
/**
 * HTTP client for {@code POST /api/banking/currency/deposit/prepare}, structurally identical to
 * {@link BankingWithdrawalClient} in both senses: the same three injectable seams and production
 * collaborators for its one prepare action, and the same composition for {@code
 * confirm}/{@code cancel} -- those two are entirely operation-type-agnostic on the wire (Rails'
 * own docs for the currency endpoints state confirm/cancel are unchanged, with no
 * operation-type branching), so this class delegates them to a composed {@link
 * BankingDepositClientPort} rather than duplicating already-tested dispatch logic a third time.
 */
public final class BankingCurrencyDepositClient implements BankingCurrencyDepositClientPort {
    ...
    public BankingCurrencyDepositClient() {
    ...
    public CompletableFuture<BankingCurrencyDepositPrepareResult> prepareCurrencyDeposit(
        ...
        return dispatch(
                server, RailsApiUrlResolver.Endpoint.BANKING_CURRENCY_DEPOSIT_PREPARE, serializePrepare(request),
                BankingTransferResponseParser::parseCurrencyDepositPrepare,
                BankingCurrencyDepositPrepareResult.LocalFailure::new, BankingCurrencyDepositPrepareResult.TransportFailure::new
        );
    }
    ...
    private static byte[] serializePrepare(BankingCurrencyDepositPrepareRequest request) {
        JsonObject currency = new JsonObject();
        currency.addProperty("currency_key", request.currencyKey());
        currency.addProperty("amount", request.amount());
