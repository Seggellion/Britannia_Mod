...
public final class BannerRenderDataSync {
    ...
    private static void onDatapackSync(OnDatapackSyncEvent event) {
        ...
        event.getRelevantPlayers().forEach(player -> {
            // Display-only snapshot: skip connections that negotiated no britannia
            // channels (gametest mock players) instead of letting NeoForge throw.
            if (!player.connection.hasChannel(S2CBannerRenderDataPayload.TYPE)) {
                return;
            }
