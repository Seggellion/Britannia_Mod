...
## 11. Milestone 21 item-content accounting (2026-08-11)

Owner directive: create the models, textures and food mechanics for the
pending items, **being mindful of items that already exist**; weapons and
armor are already craftable and are partially added.

Root cause found first: the Milestone 2 item-universe extraction only
matched literal `register("...")` calls, so every helper-registered item
(`cookedFood` and friends — ~175 real ids) was invisible and its rows were
misclassified MISSING_ITEM. The broadened DeferredHolder-aware extraction
now lives in `validate_mapping.py`, `parse_runuo_vendors.py` and the new
`tools/reaudit_missing_items.py`, which re-audited every pending row
against the true universe: **426 rows across 188 RunUO types recovered
with zero new assets** (exact snake_case, then a reviewed alias table —
including the platemail/ringmail/chainmail armor craftables, which RunUO
names Plate*/Ringmail*/Chain*). Variable-material rows keep their varmat
status when resolved to a mod item (material machinery still prices them)
and become fixed products when resolved to a vanilla id, which cannot
carry material variants.

New content: **122 items** generated by `tools/generate_m21_items.py`
(registrations, flat item models, 16×16 textures, lang entries) — foods
with real nutrition/saturation via the existing `cookedFood` mechanics
(pies, breads, SE cuisine, soups, bowls), provisioner/tinker/carpenter
goods, textile goods, scribe goods, six gems, necromancy reagents,
clothing, and GOLD-tier jewelry through the existing material-quality
jewelry system (reusing the real jewelry textures). Two near-misses caught by
the guards: amethyst and tourmaline are NOT generated -- BlacksmithItemRegistry
already registers every craftables ingredient key as an item, and duplicating
them leaves a null registry hole (the `noItemIdIsRegisteredTwice` gametest
failed exactly there); their placeholder models were repointed at real art
instead, and the ingredient-key registrations joined the tools' item universe.

Rollout after regeneration: 33 vendor types, **31 ACTIVE** (up from 25;
rancher and shipwright stay registered-inactive pending mobile/boat
fulfillment). Retail rows (1,015): **707 seeded** (524 fixed-input + 183
material rows across 274 unique Products, 605 listings); 308 explicitly
excluded: 122 pending items, 60 magic-deferred, 30 deed/service, 29
mobile fulfillment, 26 unsupported, 41 in merged/service vendors without
a retail home. New canonical commodity families: `textile|raw|wool`,
`textile|cloth|cloth`, `scribe|paper|paper` and the five necromancy
reagents (`reagents|raw|bat_wing`, `daemon_blood`, `pig_iron`,
`nox_crystal`, `grave_dust`) — all weight-canonical bulk supply.

The 122 rows still pending are mechanics-bearing families deliberately
not faked as flat items:

| rows | family | reason |
| --- | --- | --- |
| 51 | leather/studded armor | leather armor family gap (OQ-9) |
| 23 | wood weapons (clubs, staves, SE) | wood-material machinery absent |
| 21 | ranged weapons + ammunition | bowyer/ranged family absent |
| 14 | niche systems (aquarium, bag balls, salvage, housing tools, games) | each is its own subsystem |
| 5 | skill/system books | teaching mechanics |
| 4 | whole fish | per-species fish system |
| 4 | dyes and dye tubs | dye system |
