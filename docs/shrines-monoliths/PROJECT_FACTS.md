...
| Verified repository fact | The nine stable shrine identities resolve one shared static GeckoLib geometry and nine distinct 128 by 128 PNG texture paths plus localized display names. All of these assets are explicitly temporary owner-authorized development placeholders, not final approved art. | `ShrineMonolithDefinitions.java`; `docs/shrines-monoliths/PLACEHOLDER_ASSETS.md`; packaged-resource tests |
