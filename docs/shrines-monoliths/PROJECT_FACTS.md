...
| Verified repository fact | At the audited `b261653` snapshot, the protected package contained 20 assets. The later worktree adds `light_granite.png`, bringing the current inventory to 21: 15 shrine resources and 6 monolith resources. The current clean-build thin and deployable JARs contain every protected source byte with zero missing or mismatched entry; this remains worktree evidence rather than a refreshed committed merge audit. | historical and current source/JAR SHA-256 audits; protected-asset tests |
