...
| Verified repository fact | The shrine item is server-authoritative and currently configures only the approved default Honesty diagnostic shrine. Its immutable plan captures exactly four original and expected block states before mutation. | `structure/item/ShrineItem.java`; `structure/placement/ShrinePlacementPlanner.java` |
...
Verified repository fact: no current-branch implementation provides all of anchor ownership, persisted footprint, preflight, atomic placement, rollback, centralized teardown/drop/pick/explosion/piston policy, unloaded-chunk deferral, orphan repair, and reentrancy protection.
...
| Classification | Finding | Evidence |
| --- | --- | --- |
| Verified repository fact | Milestone 2 adds diagnostic shrine placement through one invisible anchor, three invisible parts, one in-memory anchor state, and a family item. It adds no final shrine renderer/texture, monolith placement, decorator integration, persisted saved-state schema, lifecycle repair, or configured structure drops. | `src/main/java/com/seggellion/britannia_mod/structure`; registration/resource inventories; scope tests |
