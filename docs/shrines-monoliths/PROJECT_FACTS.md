...
| Verified historical audit fact | At the original Milestone 10 audit, the catalogue contained nine shrine variants and two provisional monolith variants; collision was `SOLID_CELL`, footprint sizes were 4 and 18, and both monolith render offsets were `[0,16,0]` voxels. Later owner-authorized content is recorded below. | original definitions; deterministic content report/tests |
| Verified historical audit fact | The original Milestone 10 protected structure-asset inventory was 19 files (13 shrine, 6 monolith). Later corrective and owner-authorized additions supersede that count without rewriting the historical audit. | historical `MERGE_READINESS.md`; source/JAR hash comparison |
...
| Verified repository fact | The owner-authorized provisional monolith catalogue contains exactly three enabled player-facing runtime variants in explicit cycle order: `diagnostic_missing_content`, `diagnostic_alternate`, `diagnostic_crystalline`, then wrap. Each owns a distinct static geometry and opaque 32 by 32 texture. All three reuse the one technically required empty family animation and the one vanilla-stone item model. None is final artwork. | `ShrineMonolithDefinitions.java`; `PLACEHOLDER_ASSETS.md`; resource, catalogue, and cycle tests |
| Verified repository fact | All three provisional monolith models have measured authored bounds X `[-8,40]`, Y `[-16,32]`, Z `[-8,24]` voxels, pivot `[0,-16,0]`, North default forward, and an asymmetric front marker at negative Z. The anchor renderer applies `[0,+1,0]` blocks exactly once; their finite all-facing union bounds remain X/Z `[-2,3]`, Y `[0,3]`, plus `1/128` tolerance. | all `monolith_diagnostic*.geo.json` files; `ShrineRenderer.java`; `ShrineRenderTransform.java`; rendering tests |
| Unresolved question | Final monolith names, replacement art, and migration policy remain unresolved for later final-content work; the exact three provisional variants are the current owner-approved package. | `docs/shrines-monoliths/OPEN_QUESTIONS.md` |
...
## Corrective Milestone 9A facts

| Classification | Finding | Evidence |
| --- | --- | --- |
| Owner live evidence | The four logical cells and cell-bounded collision were correct, but the dynamic shrine was shifted into the surrounding oak-stair frame, left part of the intended opening visually empty, and showed a thin overlapping stripe. | Owner screenshot description and corrective authorization |
| Verified root cause | GeckoLib mirrors Bedrock model X before its anchor-center translation. The old raw X center was `-8` voxels, which became renderer X `+0.5` block; rotating that offset around the anchor center displaced the presentation by one block in a facing-dependent horizontal axis. Renderer translation `(0.5, 0, 0.5)`, scale `1`, rotations, and shrine Y offset `0` were correct and remain unchanged. | Resolved GeckoLib 4.6.6 bytecode; `ShrineRenderTransform`; corrective regression fixture |
| Verified corrected geometry | The owner-supplied original model supersedes the three-step silhouette. Shared geometry has four equal `14 x 14 x 14` surface quadrants plus four non-overlapping 15-voxel-high, 2-voxel-thick granite rim pieces; raw union is X/Z `[-8, 24]`, Y `[0, 15]`. The 32-voxel exterior exactly fills the logical `2 x 2` footprint, effective horizontal size `2 x 2` blocks, and vertical range `0..0.9375` block. The top-face UV rows remain swapped relative to raw Z so the texture's upper half renders in the upper two world cells instead of wrapping below its lower half. | `assets/britannia_mod/geo/shrine.geo.json`; geometry/UV regression test; owner gap screenshots |
| Verified envelope contract | North effective bounds are X `[-1, 1]`, Z `[0, 2]`; East X/Z `[-1, 1]`; South X `[0, 2]`, Z `[-1, 1]`; West X/Z `[0, 2]`, all relative to the anchor and all at Y `[0,0.9375]`. Each exactly matches the horizontal boundary of its four-cell union without entering any of the eight adjacent perimeter cells. | Production transform helpers and parsed-geometry/stair-frame tests |
| Verified rendering scope | One anchor renderer remains and adds one bounded material layer that renders only the `granite_rim` bone. That layer selects `light_granite.png` only for synchronized `shrine/chaos` state and `granite.png` for the other eight shrine variants and every fail-closed identity. Anchor and parts retain invisible baked presentation; parts have no block entity or renderer. Collision, selection, placement, persistence, lifecycle, integrity, cycling, monolith rendering, and serialized schemas are unchanged. | `ShrineRenderer`; `ShrineGraniteRenderLayer`; `ShrineRimMaterialSelection`; selector tests |
...
| Verified assets | Owner-supplied shrine geometry, nine 128 x 128 virtue textures, `granite.png`, and the new 128 x 128 `light_granite.png` are the current shrine package. The flush-envelope geometry SHA-256 is `DAEF7813658A6EB8C9831D538599EB58892E184174896D4B2179E21C29C099FE`; both granite files currently have SHA-256 `A1D3C1A881B6DC6990EB56932B702CDA78AE0BBF10355FDA90B8A3133B4CCA77` and are byte-identical. The distinct chaos resource path is therefore implemented but will not produce a visible material difference until distinct approved bytes are supplied at that path. Exact per-variant and rim mappings are enforced by tests and `CONTENT_REPORT.json`. The owner `old/` backup is excluded from packaged resources. | protected-asset tests; `ShrineRimMaterialSelectionTest`; `CONTENT_REPORT.json`; `build.gradle` resource exclusion |
...
| Verified repository fact | At the audited `b261653` snapshot, the protected package contained 20 assets. The later shrine work raised the inventory to 21, and the owner-authorized crystalline model/texture raise the current inventory to 23: 15 shrine resources and 8 monolith resources. Refreshed package evidence is recorded in the implementation log and remains separate from the historical merge audit. | historical and current source/JAR SHA-256 audits; protected-asset tests |
