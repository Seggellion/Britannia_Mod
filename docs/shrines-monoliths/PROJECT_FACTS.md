...
| Verified corrected geometry | Shared geometry remains one static three-cube model. Raw union is X/Z `[-7, 23]`, Y `[0, 16]`; GeckoLib-space center is `(-0.5, +0.5)` relative to the anchor-center pivot. The 30-voxel base yields a symmetric one-voxel edge inset, effective horizontal size `1.875 x 1.875` blocks, and vertical range `0..1` block. | `assets/britannia_mod/geo/shrine.geo.json`; `CorrectiveMilestoneNineARenderAlignmentTest` |
| Verified envelope contract | North effective bounds are X `[-15/16, 15/16]`, Z `[1/16, 31/16]`; East X/Z `[-15/16, 15/16]`; South X `[1/16, 31/16]`, Z `[-15/16, 15/16]`; West X/Z `[1/16, 31/16]`, all relative to the anchor and all at Y `[0,1]`. Each is centered in and contained by its exact four-cell union and intersects none of the eight perimeter cells. | Production transform helpers and parsed-geometry/stair-frame tests |
| Verified rendering scope | One anchor renderer remains; anchor and parts retain invisible baked presentation; parts have no block entity or renderer. Collision, selection, placement, persistence, lifecycle, integrity, cycling, monolith rendering, and serialized schemas are unchanged. | Existing Milestone 1-9 tests plus corrective scope assertions |
| Verified Creative exposure | Existing `britannia_mod:britannia_decor_tab` is reused. It accepts exactly one explicitly configured `britannia_mod:shrine` stack through `ShrineItemStateAccess.defaultState()`, resolving to `shrine/honesty`. No new tab, item registration, variant entry, anchor/part item, monolith entry, or recipe exists. | `CreativeTabRegistry`; `LargeStructureRegistry`; corrective Creative test |
| Verified assets | Only shared shrine geometry changed, from SHA-256 `C31915013A7D50D1732225764D4F94FEB1AD141515BAC0F3CADC81E5C8B3BCA0` to `05C52F184101C5AA62EEE515EB3BB285975FAAE2744EAC3381512000F0EEE62E`. All other 18 protected shrine/monolith resources remain byte-identical. | protected-asset test; source/JAR comparison; `CONTENT_REPORT.json` |
| Explicitly pending | Headless parsing, registered stair-state checks, and client resource startup do not prove the final GPU image. Owner confirmation for four facings, three representative texture variants, Creative exposure, collision, and save/reload remains required; Milestone 10 is suspended. | `OPEN_QUESTIONS.md`; Corrective Milestone 9A implementation log |
