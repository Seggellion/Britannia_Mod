...
| Verified repository fact | The current branch has plain-JUnit coverage through narrow world/mutation adapters plus actual registered blocks/items/components and actual NBT/update-tag/packet-application APIs. Milestone 5 adds Interior Decorator authorization, transaction, rollback, membership, persistence, immutable-asset, and dispatch-scope tests. It still has no `@GameTest`, client interaction suite, or CI workflow; live gameplay remains unverified. | source/docs/`.github` searches; Milestone 5 tests |
