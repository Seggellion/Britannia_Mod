...
| Verified corrected geometry | The owner-supplied original model supersedes the three-step silhouette. Shared geometry has four equal `13 x 14 x 13` surface quadrants plus four non-overlapping 15-voxel-high granite rim pieces; raw union is X/Z `[-7, 23]`, Y `[0, 15]`. The 30-voxel exterior retains the symmetric one-voxel footprint inset, effective horizontal size `1.875 x 1.875` blocks, and vertical range `0..0.9375` block. The top-face UV rows are swapped relative to raw Z so the texture's upper half renders in the upper two world cells instead of wrapping below its lower half. | `assets/britannia_mod/geo/shrine.geo.json`; geometry/UV regression test; owner screenshot |
| Verified envelope contract | North effective bounds are X `[-15/16, 15/16]`, Z `[1/16, 31/16]`; East X/Z `[-15/16, 15/16]`; South X `[1/16, 31/16]`, Z `[-15/16, 15/16]`; West X/Z `[1/16, 31/16]`, all relative to the anchor and all at Y `[0,0.9375]`. Each is centered in and contained by its exact four-cell union and intersects none of the eight perimeter cells. | Production transform helpers and parsed-geometry/stair-frame tests |
