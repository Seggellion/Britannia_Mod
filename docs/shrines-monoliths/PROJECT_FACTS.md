...
| Verified repository fact | Shrine cycling is gated on the logical server by the Milestone 5 fallback policy centralized in `DecoratorAuthorization`: server-validated creative mode or server permission level 2 or higher. Unrelated decorator targets retain their previous authorization behavior. | `DecoratorAuthorization.java`; `InteriorDecoratorToolItem.java`; authorization tests |
...
| Verified repository fact | Milestone 5 inserts shrine dispatch before generic horizontal rotation. An authorized click on a valid anchor or part uses the existing lifecycle resolver, changes only persisted `variant_id`, synchronizes the anchor through its existing update path, plays the vanilla stonecutter-selection sound once, and sends localized action-bar key `message.britannia_mod.shrine.decorator.selected`. | `InteriorDecoratorToolItem.java`; `ShrineVariantCycleService.java`; transaction and scope tests |
...
| Verified repository fact | The owner-authorized provisional monolith package contains exactly one enabled player-facing runtime variant, `diagnostic_missing_content`: one static geometry, one opaque 32 by 32 stone texture, one technically required empty animation resource, and one vanilla-stone item model. It is explicitly not final artwork. | `ShrineMonolithDefinitions.java`; `PLACEHOLDER_ASSETS.md`; resource and catalogue tests |
| Verified repository fact | Authored monolith bounds are X `[-8,40]`, Y `[-16,32]`, Z `[-8,24]` voxels with pivot `[0,-16,0]`, North default forward, and front at negative Z. The anchor renderer applies `[0,+1,0]` blocks exactly once; finite all-facing bounds are X/Z `[-2,3]`, Y `[0,3]`, plus `1/128` tolerance. | `monolith_diagnostic.geo.json`; `ShrineRenderer.java`; `ShrineRenderTransform.java`; rendering tests |
| Unresolved question | Final monolith variant identities and owner-approved replacement art remain unresolved and block Milestone 7, not Milestone 6; see `OPEN_QUESTIONS.md`. | `docs/shrines-monoliths/OPEN_QUESTIONS.md` |
...
| Verified repository fact | The current branch has plain-JUnit coverage through narrow world/mutation adapters plus actual registered blocks/items/components and actual NBT/update-tag/packet-application APIs. Milestone 6 adds eighteen-cell placement, rollback, lifecycle, integrity, item, persistence, renderer, resource, and immutable-shrine-asset coverage. It still has no `@GameTest`, client interaction suite, or CI workflow; live gameplay and controlled visual-horizon alignment remain unverified. | source/docs/`.github` searches; Milestone 6 tests |
