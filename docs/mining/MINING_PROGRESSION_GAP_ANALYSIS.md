...
| Silver | 55.0 | PARTIAL | `silver_ore` block+item+full assets+lang; `silver_ingot` item+model; `SILVER` tool material; `VerticalLayeredVein` wired; Rails `ore/raw/silver` + `metal/ingots/silver`; restoration generic | **0 silver veins in Rails seeds** (not obtainable in a fresh world until vein rows exist + `/populateores silver`); ingot texture is an iron-ingot placeholder; no ingot lang; E1 sale-identity conflict; no tests |
...
| Mining skill award | MISSING | Zero `SkillManager` calls in mining; Rails Mining row exists; per-skill difficulty modifier gives a clean 18k-calibration lever (discovery §3.1) |
| Skill engine (shared) | COMPLETE | `SkillManager` + Rails persistence + sync + states + trainer; reuse as-is |
| Guildmaster Mining training | COMPLETE (Rails side) | `miner` guild teaches Mining; existing purchase flow is skill-agnostic |
...
| Ore economy identity | DUPLICATE/CONFLICT (unverified live) | E1/E2 exact-lookup mismatches vs current seeds; live DB shape UNKNOWN — must be verified against the running Rails before M8 wiring |
