...
After the clean release, the exact corrected candidate was also launched in the isolated no-Iris/Sodium client for a log-based startup check. It completed atlas/model resource and entity-animation loading and remained running, then only that owned process was closed. Its installed JAR hash matched the archive. `tmp/gameplay-bugfixes/qa-followup/client-candidate-startup.log` records this startup check; no world scene or visual result is inferred from it.
