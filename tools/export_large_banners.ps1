param(
    [string]$IllustratorSource = 'C:\projects\britannia\raw fiels\tabbard\banner_large.ai',
...
$bannerSpecs = @(
    @{ StableId = 'tournament_curtain'; Layer = 'tournament_curtain'; PreviousId = 'large_01'; CatalogueIndex = 1 },
    @{ StableId = 'threefold_chain_standard'; Layer = 'threefold_chain_standard'; PreviousId = 'large_02'; CatalogueIndex = 2 },
    @{ StableId = 'iron_serpent_standard'; Layer = 'iron_serpent_standard'; PreviousId = 'large_03'; CatalogueIndex = 3 },
    @{ StableId = 'silver_fleur_curtain'; Layer = 'silver_fleur_curtain'; PreviousId = 'large_04'; CatalogueIndex = 4 },
    @{ StableId = 'gilded_trellis_curtain'; Layer = 'gilded_trellis_curtain'; PreviousId = 'large_05'; CatalogueIndex = 5 },
    @{ StableId = 'gilded_chevron_curtain'; Layer = 'gilded_chevron_curtain'; PreviousId = 'large_06'; CatalogueIndex = 6 }
)
...
try {
    ...
    $report.document = [ordered]@{
        ...
        clipping_mask_observation = 'No inspected named Large page item reports Clipped=true.'
    ...
    if (Compare-Object -ReferenceObject $expectedLayers -DifferenceObject $actualLayers) {
        throw 'Authoritative Illustrator membership differs from the six discovered Large candidates'
    }
    ...
    foreach ($bannerSpec in $bannerSpecs) {
        ...
        $report.reconciliation += [ordered]@{
            ...
            existing_catalogue_id = "britannia_mod:$($bannerSpec.PreviousId)"
            existing_group = 'large'
            catalogue_index = [int]$bannerSpec.CatalogueIndex
            mapping_evidence = 'Exact authoritative source name, one-to-one six-layer Large membership matching the six provisional Large slots, and the authored layer sequence matching the catalogue page/row sequence; the mapping is not based on anonymous layer order alone.'
            orientation_evidence = 'The task identifies banner_large.ai as the authoritative parallel Large source and the catalogue intake gate independently fixes the large group to wall_parallel only.'
            result = 'MIGRATE_PROVISIONAL_LARGE_ID'
        }
...
$reportPath = Join-Path $RepositoryRoot 'content\banner-final-intake\large_illustrator_report.json'
